...
namespace FunkyCode
{
	public class LightingRender2D
	{
		...
		public static Vector3 GetSize(Camera camera) {
			...
			// Convert bottom-left and top-right corners of the pixelRect into world space
			Vector3 bl = camera.ScreenToWorldPoint(new Vector3(r.xMin, r.yMin, camera.nearClipPlane));
			Vector3 tr = camera.ScreenToWorldPoint(new Vector3(r.xMax, r.yMax, camera.nearClipPlane));

			// Size is half the full width/height of the camera's view in world units
			Vector3 size = (tr - bl) * 0.5f;

			// Slightly expand the size outward by half a pixel (in world units) to eliminate rounding gaps
			
			float halfPixel = 0.5f / 8;
			size.x += halfPixel;
			size.y += halfPixel;
